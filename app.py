        ...
        if submit_btn:
            ...
            try:
                ...
                if predicted_score < 60.0:
                    st.error("🚨 **High Academic Risk (Score < 60):** Predicted metrics fall into the critical performance track. Immediate structural intervention highly recommended.")
                elif predicted_score <= 70.0:
                    st.warning("⚠️ **Moderate Risk (Score 60 - 70):** Predicted metrics fall into the standard pass baseline track. Progress stable, monitor closely to prevent down-drifts.")
                else:
...
with tab3:
    ...
    if uploaded_file is not None:
        try:
            ...
            # Calibrated thresholds (<60, 60-70, >70)
            def classify_risk_um(score):
                if score < 60.0: return "High Risk (<60)"
