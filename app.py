        ...
        if submit_btn:
            ...
            try:
                ...
                if predicted_score < 50.0:
                    st.error("🚨 **High Academic Risk (Score < 60):** Predicted metrics fall into the lower fail track band. Immediate structural intervention highly recommended.")
                elif predicted_score <= 70.0:
                    st.warning("⚠️ **Moderate Risk (Score 60 - 70):** Predicted metrics fall into the pass/good track baseline band. Progress stable, monitor closely to prevent down-drifts.")
                else:
                    st.success("✨ **Low Risk (Score > 70):** Predicted metrics reflect strong performance trending in the distinction tier. Maintain current metrics!")
            except Exception as e:
...
with tab3:
    ...
    if uploaded_file is not None:
        try:
            ...
            # Updated threshold classification mapping
            def classify_risk_um(score):
                if score < 50.0: return "High Risk (<60)"
                elif score <= 70.0: return "Moderate Risk (60-70)"
            ...
            high_risk_count = int(sum(df_batch['Risk_Classification'] == "High Risk (<60)"))
            mod_risk_count = int(sum(df_batch['Risk_Classification'] == "Moderate Risk (60-70)"))
            low_risk_count = int(sum(df_batch['Risk_Classification'] == "Low Risk (>70)"))

            m1, m2, m3 = st.columns(3)
            m1.metric("🚨 High Risk (Score < 60)", f"{high_risk_count} students")
            m2.metric("⚠️ Moderate Risk (Score 60-70)", f"{mod_risk_count} students")
            m3.metric("✨ Low Risk (Score > 70)", f"{low_risk_count} students")

            chart_col1, chart_col2 = st.columns(2)
            academic_order = ["Low Risk (>70)", "Moderate Risk (60-70)", "High Risk (<60)"]
...
# AUTOMATED TERMINAL ROUTING SYSTEM (COPY FILE TO GOOGLE DRIVE)
import os
import shutil
drive_dest = '/content/drive/MyDrive/Data_Analytics/Datasets/'
shutil.copy('app.py', os.path.join(drive_dest, 'app.py'))
print("[STATUS 4/4] app.py -> Written to Drive with Plotly layouts enabled!")
print("\n✨ [SUCCESS] All 4 deployment assets are now successfully saved to Google Drive with updated threshold maps!")
